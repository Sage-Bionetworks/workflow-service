...
class Workflow(object):
    ...
    def run(self, request, tempdir, opts):
        ...
        Runner:
        opts.getopt("runner", default="nextflow")
        ...
        nextflow_config = os.path.join(request["workflow_attachment"], "nextflow.config")
        nextflow_config = nextflow_config.replace("file://", '')
        # link the cwl and json into the tempdir/cwd
        if workflow_url.startswith('file://'):
            os.symlink(workflow_url[7:], os.path.join(tempdir, "wes_nextflow.nf"))
            workflow_url = os.path.join(tempdir, "wes_nextflow.nf")
        # os.symlink(inputtemp.name, os.path.join(tempdir, "cwl.input.json"))
        # jsonpath = os.path.join(tempdir, "cwl.input.json")
        
        # build args and run
        command_args = [runner] + extra + [workflow_url, '-c', nextflow_config]
...
class NextflowRunnerBackend(WESBackend):
    def GetServiceInfo(self):
        runner = self.getopt("runner", default="nextflow")
...
def create_backend(app, opts):
    return NextflowRunnerBackend(opts)
